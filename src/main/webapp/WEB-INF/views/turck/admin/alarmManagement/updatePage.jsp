    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-3">
                        <form id="updateAlarmMasterListForm" action="<%=request.getContextPath()%>/turck/admin/alarmManagement/updateAlarmMasterList" method="post">
                            <div class="card card-primary card-outline">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="locationCode" class="form-control" value="">
                                    ...
                                    <div id="deviceCodeDiv" class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="deviceCode" class="form-control" value="">
                                    ...
                                    <div id="sensorCodeDiv" class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="sensorCode" class="form-control" value="">
                                                <option value="">- 센서 선택 -</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div id="tagCodeDiv" class="form-group row">
                                        <label for="tagCode" class="col-sm-3 col-form-label">태그 :</label>
                                        <div class="col-sm-9">
                                            <select id="tagCode" class="form-control" value="">
                            ...
                            <div class="card card-warning card-outline">
                                <div class="card-header">
                                    <h3 class="card-title">필수입력 조건</h3>
                                </div>
                                <div class="card-body">
                                    <div class="form-group row">
                                        <label for="warningValue" class="col-sm-3 col-form-label">경고 값 :</label>
                                        <div class="col-sm-9">
                                            <input id="warningValue" data-validate="null" autocomplete="off" type=number class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="alarmValue" class="col-sm-3 col-form-label">알람 값 :</label>
                                        <div class="col-sm-9">
                                            <input id="alarmValue" data-validate="null" autocomplete="off" type=number class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <!-- /.card-body -->
                        ...
                        <a id="updateAlarmMasterListBtn" class="btn btn-warning btn-block mb-3">알람 목록 수정</a>
                    </div>
                    <div class="col-9">
                        <div class="card card-primary card-outline">
                            <div class="card-header">
...
<script>
    $(function () {
        $(document).ready(function () {
            ...
            commonIncludeLeftMenuImageChange("Child", "alarmManagementUpdatePage");
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta){
                        return '<input type="checkbox" name="alarmMasterListCheck" value="' + $('<div/>').text(data).html() + '">';
                    }
                },
                {targets: 1, width: "4%", className: "text-center"},
                {targets: [5], width: "22%", className: "text-center"},
                {targets: [2, 3, 4], width: "10%", className: "text-center"},
                {targets: 6, width: "24%", className: "text-center"},
                {targets: [7, 8], width: "9%", className: "text-center"}
            ],
            order: [1, 'asc']
        });
    ...
    function selectAlarmMasterList() {
        const action = "<%=request.getContextPath()%>/turck/admin/alarmManagement/selectAlarmMasterList";
    ...
    $("#updateAlarmMasterListBtn").click(function() {

        const alarmMasterList = new Array();
        const checkbox = $("input[name=alarmMasterListCheck]:checked");
        ...
        if(checkbox.length == 0) {
            ...
            if (!$.fn.validate($("#updateAlarmMasterListForm"))) {
                return false;
            }

            confirmPopUp("알람 정보 수정", "알람 정보를 수정 하시겠습니까?", function() {
                checkbox.each(function (i) {
                    const tr = checkbox.parent().parent().eq(i);
                    const td = tr.children();

                    const alarmCode = td.eq(5).text();

                    const warningValue = $("#warningValue").val();
                    const alarmValue = $("#alarmValue").val();

                    alarmMasterList.push({"alarmCode": alarmCode, "warningValue": warningValue, "alarmValue": alarmValue});
                });

                updateAlarmMasterList(JSON.stringify(alarmMasterList));
            });
        }
    });

    function updateAlarmMasterList(alarmMasterList) {
        loadingStart("알람 정보 수정 중 입니다. 잠시만 기다려주세요.");

        const action = $("#updateAlarmMasterListForm").attr('action');

        const warningValue = $("#warningValue").val();
        const alarmValue = $("#alarmValue").val();

        const form_data = {
            alarmMasterList: alarmMasterList,
        ...
        $.ajax({
            ...
            error: function (jqXHR, resultData) {
                alarmPopUp(2, "알람 마스터 수정 중 [" + jqXHR.status + "] 오류가 발생했습니다.<br>관리자에게 문의하세요.", "");
            },
            complete: function () {
                selectAlarmMasterList();
