    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-3">
                        <form id="deleteDeviceMasterListForm"
                              action="<%=request.getContextPath()%>/turck/admin/deviceManagement/deleteDeviceMasterList"
                              method="post">
                        ...
                        <div class="row">
                            <div class = "col-12">
                                <a id="deleteDeviceMasterListBtn" class="btn btn-danger btn-block mb-3">디바이스 정보 삭제</a>
                            </div>
                        </div>
...
<script>
    $(function () {
        $(document).ready(function () {
            ...
            commonIncludeLeftMenuImageChange("Child", "deviceManagementDeletePage");
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta){
                        return '<input type="checkbox" name="deviceMasterListCheck" value="'
                            + $('<div/>').text(data).html() + '">';
                    }
                },
                {targets: 1, width: "2%", className: "text-center"},
                {targets: [3, 5, 6], width: "32%", className: "text-center"},
    ...
    $("#deleteDeviceMasterListBtn").click(function() {
        const deviceMasterList = new Array();
        const checkbox = $("input[name=deviceMasterListCheck]:checked");

        if(checkbox.length == 0) {
            alarmPopUp(2, "디바이스 목록을 선택하지 않았습니다. <br>디바이스 목록 선택 후 다시 시도해주세요.", "");

            return;
        } else {
            confirmPopUp("디바이스 정보 삭제", "디바이스 정보를 삭제 하시겠습니까? <br><H7 style='color:#dc3545; font-weight:bold'> * 디바이스 목록 삭제 시 하위 센서 및 태그의 정보도 삭제 됩니다.</H7>", function() {
                checkbox.each(function (i) {
                    const tr = checkbox.parent().parent().eq(i);
                    const td = tr.children();

                    const deviceCode = td.eq(3).text();

                    deviceMasterList.push({"deviceCode": deviceCode});
                });

                deleteDeviceMasterList(JSON.stringify(deviceMasterList));
            });
        }
    });

    function deleteDeviceMasterList(deviceMasterList) {
        loadingStart("디바이스 정보 삭제 중 입니다. 잠시만 기다려주세요.");

        const action = $("#deleteDeviceMasterListForm").attr('action');

        const form_data = {
            deviceMasterList: deviceMasterList
        };
        ...
        $.ajax({
            ...
            error: function (jqXHR, resultData) {
                alarmPopUp(2, "디바이스 목록 삭제 중 [" + jqXHR.status + "] 오류가 발생했습니다.<br>관리자에게 문의하세요.", "");
            },
            complete: function () {
                selectDeviceMasterList();
